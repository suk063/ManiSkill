...
from map_rl_from_example.utils.mapping import update_map_online
...
class Args:
    ...
    """robot segmentation id for online mapping"""
    map_update_freq: int = 5
    """the frequency of online map updates"""
    camera_uids: List[str] = field(default_factory=lambda: ["hand_camera"])
...
if __name__ == "__main__":
    ...
    if args.use_map:
        ...
        if args.start_condition_map or args.use_local_fusion or args.use_online_mapping:
            all_grids = []
    ...
    # env setup
    if args.use_local_fusion or args.use_online_mapping:
        env_kwargs = dict(obs_mode="rgb+depth+segmentation", render_mode=args.render_mode, sim_backend="physx_cuda", camera_uids=args.camera_uids)
    else:
    ...
    envs = FlattenRGBDObservationWrapper(envs, rgb=True, depth=True, include_segmentation=True, state=args.include_state, include_camera_params=True)
    eval_envs = FlattenRGBDObservationWrapper(eval_envs, rgb=True, depth=True, include_segmentation=True, state=args.include_state, include_camera_params=True)
    ...
    if args.use_map and (args.start_condition_map or args.use_local_fusion or args.use_online_mapping):
    ...
    if args.use_map and args.start_condition_map:
        ...
        if args.use_online_mapping:
            decoder_params_ids = set(map(id, decoder.parameters()))
            base_params = [p for p in base_params if id(p) not in decoder_params_ids]
        ...
        if args.use_online_mapping:
            decoder_params_ids = set(map(id, decoder.parameters()))
            params_to_update = [p for p in params_to_update if id(p) not in decoder_params_ids]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    ...
                    action = agent.get_action(eval_obs, env_target_obj_idx=eval_target_obj_idx, map_features=online_eval_grids if args.use_online_mapping else eval_grids, deterministic=True)
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                decoder.eval()
                action, logprob, _, value = agent.get_action_and_value(next_obs, env_target_obj_idx=next_env_target_obj_idx, map_features=online_grids if args.use_online_mapping else grids)
        ...
        with torch.no_grad():
            ...
            decoder.eval()
            next_value = agent.get_value(next_obs, env_target_obj_idx=next_env_target_obj_idx, map_features=online_grids if args.use_online_mapping else grids).reshape(1, -1)
